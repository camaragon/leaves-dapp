...
function App() {
  ...
  // Backwards-compatible blockchain object for child components
  ...
  const getData = () => {
    if (isConnected && smartContract) {
      dispatch(fetchData(smartContract));
    }
  };
  ...
  const getConfig = async () => {
    ...
    SET_CONFIG(await configRes.json());
    setAbi(await abiRes.json());
  };

  useEffect(() => { getConfig(); }, []); // eslint-disable-line
